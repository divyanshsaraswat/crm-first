...
import { insertSetting,insertUserFollowUp,getFollowUp,getUserWhatsApp,updateUserWhatsapp,getUsers,sendMail,getUserById,deleteUser,checkcmd,updateById,insertUser,downloadCSV,updateSettings,forgotpassword,getAllByUserIdNotif,getDetails,getRoles,getSettings,addLogs,login,getSignedDetails,deleteByIdNotif,markAsReadNotif,createNotif,sendTrialMail} from '@/lib/services/userService';
import { cookies } from 'next/headers';
import cron from 'node-cron';
export const userController = {
  getFollowUp:async(request)=>{
    try {
      const users = await getFollowUp();
  ...
  insertUserFollowUp : async(request)=>{
    try {
      const {pid,role,tenantid} = request
      const {id,ftype,message} = await request.json()
      console.log(id,ftype,message)
      const users = await insertUserFollowUp({id:id,fid:ftype,message:message});
      const log = await addLogs({ pid, tenantid, action:message, role });
      return Response.json({ message: users }, { status: 200 });
    } catch (error) {
      return Response.json({ error: 'Internal Server Error' }, { status: 500 });
    }
  ...
  sendTrialMail:async (request)=>{
    try {
      const body = await request.json();
      const { email } = body;
      // const result = await sendTrialMail({ email:email });
  ...
  getUserWhatsApp: async (request) => {
    try {
      const { pid } = request;
      const result = await getUserWhatsApp(pid);
      return Response.json(result, { status: 200 });
    } catch (error) {
      return Response.json({ message: "Internal Server Error" }, { status: 500 });
    }
  },
  updateUserWhatsapp : async(request)=>{
    try {
      const { pid } = request;
      const body = await request.json();
      const {wid,watoken} = body;
      const result = await updateUserWhatsapp({pid,wid,watoken});
      return Response.json(result, { status: 200 });
    } catch (error) {
            return Response.json({ message: "Internal Server Error" }, { status: 500 });
  ...
  insertUser: async (request) => {
    ...
    const {username,email,password,userrole} = body;
     ...
     const mailresult = await sendMail({ sender: email,type:'userR',username:username,company:tenant_name });
     const createNotification = await createNotif({title:"User created.",message:"Welcome to your account!",type:"general",user_id:result})
