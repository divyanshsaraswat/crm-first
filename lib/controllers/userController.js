...
import { getUsers,sendMail,getUserById,deleteUser,checkcmd,updateById,insertUser,downloadCSV,updateSettings,forgotpassword,getAllByUserIdNotif,getDetails,getRoles,getSettings,addLogs,login,getSignedDetails,deleteByIdNotif,markAsReadNotif,createNotif } from '@/lib/services/userService';
...
export const userController = {
  ...
  updateById: async (request) => {
    try {
      const { pid, role,custombody } = request;
      const { id, username, email, role: userRole } = custombody;
      const result = await updateById({ id, username, email, role: userRole });
      const log = await addLogs({ pid, action: "Updated user details", role });
  ...
  downloadCSV: async (request) => {
    try {
      ...
      const log = await addLogs({ pid, action: "Downloaded users CSV", role });
  ...
  insertUser: async (request) => {
    try {
      const { pid, role, tenantid,custombody} = request;
      // const { username, email, password, userrole } = custombody;
      ...
      // const log = await addLogs({ pid, action: "Created new user", role });
