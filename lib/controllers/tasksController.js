...
import { getTasks, downloadCSV, getTaskById, insertTask, updateTask, deleteTask,updateTaskbyId, getTasksAll } from '@/lib/services/tasksService';
...
export const tasksController = {
  ...
  getTasks: async (request) => {
    try {
      const { pid, role, tenantid } = request;
      const tasks = await getTasks({pid});
  ...
  getTasksAll : async (request)=>{
    try {
      const { pid, role, tenantid } = request;
      const tasks = await getTasksAll({pid});
      const log = await addLogs({ pid, tenantid, action: "Retrieved tasks list", role });
      return Response.json({ message: tasks }, { status: 200 });
    } catch (error) {
      return Response.json({ error: 'Internal Server Error' }, { status: 500 });
