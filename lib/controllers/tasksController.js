...
import { addLogs, createNotif, sendMail } from '@/lib/services/userService';
...
export const tasksController = {
  ...
  insertTask: async (request) => {
    try {
      ...
      const mailresult = await sendMail({ sender: resultnew.email,type:"tasksC" });
  ...
  updateById: async (request) => {
    try {
      ...
      const resultnew = await getAccountById({tenantid:tenantid,id:contact_id})
      const sendMailer = await sendMail({sender:resultnew.email,type:"tasksU"})
